...
uint* wbImport(const char* fName, int* numElements)
{
    ...
    inFile >> *numElements;
    // std::cout << fName << *numElements << std::endl;
    ...
    uint* fBuf = (uint*)malloc(*numElements * sizeof(uint));

    if (!fBuf)
    {
        std::cerr << "Unable to allocate memory for an array of size " << *numElements * sizeof(uint) << " bytes" << std::endl;
        inFile.close();
        std::exit(EXIT_FAILURE);
    }
...
uint* wbImportChar(const char* fName, int* numElements)
{
    std::ifstream inFile(fName);
...
{
    int solnItems;
    uint* soln = wbImport(wbArg_getInputFile(args, args.argc - 2), &solnItems);
